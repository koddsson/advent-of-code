...
fn pull_is_possible(pull: &str) -> bool {
    for colors in pull.split(",") {
        let (count, color) = colors.trim().split_once(' ').unwrap();
        if color.trim() == "red" && count.parse::<i32>().unwrap() > 12 {
            println!("Red impossible. Limit is: 12 and the count is: {count}.");
            return false;
        }
        if color.trim() == "green" && count.parse::<i32>().unwrap() > 13 {
            println!("Green impossible. Limit is: 13 and the count is: {count}.");
            return false;
        }
        if color.trim() == "blue" && count.parse::<i32>().unwrap() > 14 {
            println!("Green impossible. Limit is: 14 and the count is: {count}.");
            return false;
        }
    }
    true
...
fn is_game_possible(pulls: &str) -> bool {
    for pull in pulls.split(";") {
        if !pull_is_possible(pull) {
            return false;
        }
    }
    true
